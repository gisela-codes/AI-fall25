...
def solve_backtracking(csp: CSP, var_order: Optional[List[str]]=None, use_mrv: bool=False) -> Iterable[Assignment]:
    ...
    def num_legal_values(var: str) -> int: #for mrv
        count = 0
        # print(var)
        for val in domains[var]:
            assignment[var] = val           # 1) try val
            if consistent_with_local(var, assignment):
                count += 1                  # 2) count if legal
            del assignment[var]             # 3) undo for next val
        return count

    def backtrack(idx: int):
        if idx == len(order):
            yield {v: assignment[v] for v in var_order or list(domains.keys()) if v in assignment} #reorders into original order
            return
        if use_mrv:
            # pick mrv variable in unassigned slice
            candidates = order[idx:]
            v_mrv = min(candidates, key=num_legal_values)
            j = order.index(v_mrv, idx)                 # find position of v_mrv
            order[idx], order[j] = order[j], order[idx] # swap into current slot
            v = order[idx]
        else:
            v = order[idx]
        # print(order, v)
